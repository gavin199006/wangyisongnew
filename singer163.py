...
def get_data(url):
    ...
    for singernames, singerids in get_singer(url):
        ...
        # info['歌手ID'] = singerids
        info.setdefault('歌手名字',singernames)
        info.setdefault('歌手ID',singerids)
        data.append(info)
    return data
...
def save2csv(url):
    print('保存歌手信息中...请稍后查看')
    with open('/zhangkun/PycharmProjects/wangyisong/热门歌手信息new.csv', 'a', newline='', encoding='utf-8-sig') as f:
